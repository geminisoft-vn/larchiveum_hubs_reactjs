		...
		if (!validator.validLength(documentRef.current.title, 1, 255)) {
			...
			(!name || name == "title") &&
				errors.push({
					name: "title",
					message:
						"title must be not empty and length less than 256 characters",
				});
		}
		if (
			!validator.validLength(documentRef.current.description, undefined, 255)
		) {
			isValidated = false;
			(!name || name == "description") &&
				errors.push({
					name: "description",
					message: "description length must be between 1 and 255 characters",
				});
		}
					...
					<>
						<Row>
							<Col span={24} style={{ padding: "10px 0px" }}>
								...
								>
									{t(
										"content.DOCUMENT_TAB__DOCUMENT_EDIT_DETAIL__BACK_BUTTON_LABEL"
									)}
								</Button>
								<Button
									type="primary"
								...
								>
									{t(
										"content.DOCUMENT_TAB__DOCUMENT_EDIT_DETAIL__SAVE_BUTTON_LABEL"
									)}
								</Button>
							</Col>
						</Row>
						<Row>
							<Col span={24}>
								<Card
									title={t(
										"content.DOCUMENT_TAB__DOCUMENT_EDIT_DETAIL__PAGE_TITLE"
									)}
								>
									<Row>
										<Col span={24}>
											<label style={{ fontSize: "14px", margin: "10px 0px" }}>
												{t(
													"content.DOCUMENT_TAB__DOCUMENT_EDIT_DETAIL__TITLE_INPUT_LABEL"
												)}
									...
									<Row>
										<Col span={24}>
											<Input
												...
												name="title"
												placeholder={t(
													"content.DOCUMENT_TAB__DOCUMENT_EDIT_DETAIL__TITLE_INPUT_PLACEHOLDER"
												)}
									...
									<Row>
										<Col span={24}>
											<label style={{ fontSize: "14px", margin: "10px 0px" }}>
												{t(
													"content.DOCUMENT_TAB__DOCUMENT_EDIT_DETAIL__DESCRIPTION_INPUT_LABEL"
												)}
											</label>
										</Col>
									</Row>
									<Row>
										<Col span={24}>
											<Input
												...
												name="description"
												placeholder={t(
													"content.DOCUMENT_TAB__DOCUMENT_EDIT_DETAIL__DESCRIPTION_INPUT_PLACEHOLDER"
												)}
								...
								<Editor
									...
									onChange={(e) => {
										onInputChange({
											target: {
												name: "content",
												value: e.target.getBody().innerHTML,
											},
										});
									}}
									onBlur={(e) => {
